...
```sh
git clone git@github.com:PierceLanternStudios/piercesdk-typescript.git
cd piercesdk-typescript
./scripts/bootstrap
./scripts/build
```
...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npx -y piercesdk2-mcp`
...
```json
{
  "mcpServers": {
    "piercesdk2_api": {
      "command": "node",
      "args": ["/path/to/local/piercesdk-typescript/packages/mcp-server", "--client=claude", "--tools=all"],
      "env": {
